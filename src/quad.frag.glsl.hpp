...
"float e,n;"
"r=vec3(109.,14.,86.);"
"if(0<int(uniform_array[7].b)%2&&.0<intersect_sphere(p,d,r,9.))"
"{"
...
"}"
"vec3 s=mix(vec3(.9,.8,.8),vec3(.8,.8,.9),d.g*111.*.02)*(dot(sample_noise(p*.006+d*.1),d)*smoothstep(-.2,.5,-d.g)*.2+.8);"
...
"r=p;"
"e=intersect_sphere(r,d,uniform_array[8],destruction+.2);"
"if(.0<e)output_color.rgb-=clamp(1.-(dot(r-p,r-p)-dot(P-p,P-p))*.003,.0,1.)*(1.-pow(e/destruction,5))*(dot(sample_noise((r-uniform_array[8])*.009),d)*.1+.9);"
"}"
#endif
"";
