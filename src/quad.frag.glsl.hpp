...
"float l=uniform_array[9].g;"
"bool w=1.<abs(uniform_array[7].b);"
"float f(vec3 p)"
"{"
"float a,b,c,r;"
"vec3 h,i,j,k;"
...
"j=m*i*2.11;"
"k=m*j*2.11;"
"a=texture(surface_texture,h.rb).r*2.61;"
"b=texture(surface_texture,i.rb).r*1.77;"
"c=texture(surface_texture,j.rb).r*.11;"
"r=texture(surface_texture,k.rb).r*.11;"
...
"}"
"}"
"N=normalize(vec3(f(n.rgb+march_params.rgg).r,f(n.rgb+march_params.grg).r,f(n.rgb+march_params.ggr).r)-c.x);"
"break;"
"}"
"p=n;"
"a=c;"
...
"w=!w;"
"}"
"if(w)march_params=vec4(.05,.0,.98,.022);"
...
"else"
"{"
"e=T(P+L*.5,L,march_params.a*3.,q,q);"
...
"o=vec4(mix(mix(s,vec3(1),pow(max(dot(d,L),.0),7.)),q,n),1.)-(int(gl_FragCoord.y*.5)%2+.1)*(max(max(smoothstep(.98,1.,uniform_array[7].g),smoothstep(-.02*uniform_array[9].r,.0,-uniform_array[7].g)*uniform_array[9].r),.1)+l*.02)*dot(c,c);"
"r=p;"
"e=C(r,d,uniform_array[8],l+.2);"
"if(.0<e)o.rgb-=clamp(1.-(dot(r-p,r-p)-dot(P-p,P-p))*.003,.0,1.)*(1.-pow(e/l,5))*(dot(Q((r-uniform_array[8])*.009),d)*.1+.9);"
"}"
#endif
"";
