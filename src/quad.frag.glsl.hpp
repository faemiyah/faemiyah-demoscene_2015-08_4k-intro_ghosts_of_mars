...
"return o;"
"}"
"float u(vec3 o,vec3 l,float i,out vec3 g,out vec3 f)"
"{"
"vec3 e,r;"
"float a=n(o),t,c,v=1.;"
"for(;"
...
"void main()"
"{"
"vec2 s=gl_FragCoord.xy/360.-vec2(1.78,1.);"
...
"p=vec4(mix(mix(m,vec3(1),pow(max(dot(o,i),.0),7.)),r,n),1.)-(int(gl_FragCoord.y*.5)%2+.1)*(max(max(smoothstep(.98,1.,e[7].g),smoothstep(-.02*e[9].r,.0,-e[7].g)*e[9].r),.1)+g*.02)*dot(s,s);"
"t=c;"
"v=d(t,o,e[8],g+.2);"
"if(.0<v)p.rgb-=clamp(1.-(dot(t-c,t-c)-dot(a-c,a-c))*.003,.0,1.)*(1.-pow(v/g,5))*(dot(b((t-e[8])*.009),o)*.1+.9);"
"}"
#endif
"";
